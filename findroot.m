...
while max(abs(dx)) >= parms.dxtol && max (abs (df)) >= parms.dftol% MODIFY this line to also test whether
  ...
  J = fjacobian (f, x0)  % FILL in code here to calculate the gradient or jacobian : f-prime = J = (f(x+dx)-f(x))/dx
  dx = -J\f(x) % FILL in this line to solve for the (vector) dx (change in x): x(New) = x(OLD) - f(x)/f-prime
